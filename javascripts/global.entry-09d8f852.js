...
/******/ 	__webpack_require__.p = "/source/javascripts/dist";
...
var controller = void 0; // declare outside of init so we can reset on resize
...
function initScrollMagic() {
  ...
  $('.feature:not(.hero) .feature__copy').each(function (index, element) {
    // cached variables
    var sectionCopy = element;
    var section = $(sectionCopy).closest('.feature')[0]; // select the DOM node not the object
    var sectionHeight = $(section).height();
    var sectionCopyHeightOffset = $(sectionCopy).height() + 100;

    // animation tweens
    var tweenBG = TweenMax.fromTo($(sectionCopy).parent().find('.feature__overlay'), 1, { css: { opacity: "0" } }, { css: { opacity: ".65" } });
    var tweenText = TweenMax.fromTo(sectionCopy, .5, { css: { opacity: "0" } }, { css: { opacity: "1" } });
    ...
    var sceneBG = new ScrollMagic.Scene({
      ...
      offset: windowHeight
    }).setTween(tweenBG).addTo(controller);
    ...
    var stickyBG = new ScrollMagic.Scene({
      ...
      offset: -headerHeight
    }).setClassToggle(section, 'scrolling').addTo(controller);

    var unStickyBG = new ScrollMagic.Scene({
      triggerHook: 'onEnter',
      duration: '100%',
      triggerElement: section,
      offset: sectionHeight
    }).setClassToggle(section, 'scrolling--complete').addTo(controller);
  });
}
...
$('.peekaboo').peekaboo({
  callback: function callback(e) {
    var element = e.event.currentTarget;
    var targetContent = '#' + element.getAttribute("aria-controls");
    var isExpanded = element.getAttribute("aria-expanded");
    var headerHeight = $('.header').height();

    if (isExpanded == "true") {
      // inverted check since peekaboo changes attribute before we get here...
      ...
      var featureCopy = element.closest('.feature__wrapper');
      var _offsetY = headerHeight + 20;
...
var urlPathHash = window.location.pathname + window.location.hash;
$(".header__nav a, .footer__main-nav a, .footer__resource-nav a").each(function (index, element) {
  var href = element.getAttribute("href");
  var url = window.location.href;
  var currentHref = urlPathHash.replace(/^\//, '');
...
$(document).on('click', '.js-toggle-content', function () {
  var shallWeExpand = $(this).data('expand'); // LPT: anything is possible!
  var section = $(this).data('controls');
  var $targetButton = $('[aria-controls="' + section + '"]');

  if ($targetButton && (!shallWeExpand || shallWeExpand == false || shallWeExpand && shallWeExpand == true && $targetButton.attr("aria-expanded") == 'false')) {
    $targetButton.click();
  }
});
...
(function (factory) {
  if (( false ? 'undefined' : _typeof(module)) === 'object' && _typeof(module.exports) === 'object') {
    module.exports = factory(__webpack_require__(9), window);
  } else {
...
var _peekabooToggle = __webpack_require__(3);

var _peekabooToggle2 = _interopRequireDefault(_peekabooToggle);

var _app = __webpack_require__(0);

var _app2 = _interopRequireDefault(_app);

var _scrolling = __webpack_require__(2);
...
/***/ })
/******/ ]);
//# sourceMappingURL=global.entry-09d8f852.js.map
;
